...
## ⚡ [Performance] Remove redundant UI computed properties in ScoresheetViewModel
**What:** Removed 16 redundant intermediate computed properties (e.g., `var stuntTotal: Double { scoresheet.stuntTotal.rounded2 }`) from `ScoreBin/ViewModels/ScoresheetViewModel.swift` and inlined their usage in SwiftUI views like `ScoreSummaryView`, `StickyScoreBar`, and `OverallJudgeSection`. Also removed a redundant `updatePendingCount()` call in `ScoresheetViewModel.save()`.
**Why:** To follow the architecture guideline specifying that mathematical derivations and rounding belong strictly in the UI layer without unnecessary intermediate view model allocations, and to clean up technical debt from incomplete prior PRs. The `updatePendingCount` fix removes a duplicate Main Actor background task that was already handled by `markForSync()`.
**Measured Improvement/Rationale:** Reduced boilerplate code and eliminated a redundant UI calculation pass and nested closure.
**Risk Assessment:** Very low. The behavior remains identical to the existing implementation.
